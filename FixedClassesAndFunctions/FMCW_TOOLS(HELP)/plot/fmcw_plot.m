...
maxrangedefault = 3000; % default range to crop data to (m)
p = 2; % padfactor;
                ...
                for asn = 1:length(vdats) % attenuator setting number
                    ...
                    if DoAverageBurst
                        ...
                        for ci = 1:size(vdat.vif,1)
                            chirpname(ci) = {[shotNamePrefix int2str(vdat.chirpNum(ci)) ' ' label]};
                        end
                    end
