...
if nargin==0 % run test/demo
    ...
    for ii = 1:4
        ...
        datah = plot(x,y,'b.');
        title(['menke_fit demo: slope =' num2str(slope) ', err=' num2str(er)],'interpreter','none')
        hold on
        fith = plot(x,yest,'r');
        %legend([datah,fith],'data','fit')
        th = text(1,offset,{['fit: intercept =' num2str(M_est(1)) '+/-' num2str(num2str(M_error(1)))];...
            ['fit: slope     =' num2str(M_est(2)) '+/-' num2str(num2str(M_error(2)))];...
            ['std(resid): ' num2str(std(resid))]},'verticalAl','bot');
        axis tight
        grid on
    end
    linkaxes(ax,'xy');
